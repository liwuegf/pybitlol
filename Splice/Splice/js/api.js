class DiscordWebhookManager {
    constructor() {
        ...
        this.discord = {
            clientId: 'YOUR_DISCORD_CLIENT_ID', // You need to set this
    ...
    bindEvents() {
        // Login events - Updated for OAuth
        document.getElementById('loginBtn').addEventListener('click', () => this.handleDiscordLogin());
        
        // Keep the User ID input as fallback
    ...
    checkOAuthCallback() {
        ...
        if (error) {
            this.showNotification('Discord login was cancelled or failed', 'error');
            // Clean up URL
    ...
    handleDiscordLogin() {
        const authUrl = `https://discord.com/api/oauth2/authorize?` +
            ...
            `redirect_uri=${encodeURIComponent(this.discord.redirectUri)}&` +
            `response_type=code&` +
            `scope=${this.discord.scope}`;
    ...
    async handleOAuthCallback(code) {
        ...
        try {
            ...
            const tokenResponse = await fetch('/api/discord/token', {
                ...
                body: JSON.stringify({
                    code: code,
                    redirect_uri: this.discord.redirectUri
                })
            });
            ...
            const userResponse = await fetch(`${this.discord.apiBase}/users/@me`, {
                headers: {
            ...
            this.showNotification('Failed to login with Discord. Please try again.', 'error');
        } finally {
            // Reset button state
    ...
    // Fallback manual login (keeps existing functionality)
    async handleManualLogin() {
        ...
        if (!userId) {
            this.showNotification('Please enter your Discord User ID or use Discord Login', 'error');
            return;
        }
        ...
        try {
            // Try to get user info from a Discord bot API (if you have one)
            // This would require your own backend service
            ...
            this.showDashboard();
            this.updateUserInfo();
            this.showNotification('Logged in with mock data (Discord API not configured)', 'warning');
        } finally {
            // Reset button state
            loginBtn.classList.remove('loading');
            loginBtn.querySelector('span').style.display = 'inline';
    ...
    updateUserInfo() {
        ...
        // Add additional Discord info if available
        if (this.currentUser.verified !== undefined) {
            const verifiedBadge = this.currentUser.verified ? ' ✓' : '';
            elements.profileName.textContent += verifiedBadge;
        }
    }
    ...
    async testWebhook() {
        ...
        const testMessage = {
            content: '🧪 **Test Message**\nThis is a test message from Discord Webhook Manager!',
            username: 'Webhook Tester',
            avatar_url: 'https://images.unsplash.com/photo-1518085250887-2f903c200fee?w=100&h=100&fit=crop'
        };
